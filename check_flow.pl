#!/usr/bin/perl
# Example code from http://search.cpan.org/~acferen/Net-Flow/lib/Net/Flow.pm
use strict;
use warnings;
...
my $receive_port = 8900; # IPFIX port 
my $packet; 
...
# 			  if($name =~ /Address/) {
 			  if (index($name, 'Address') ne -1) {
                                printf '  Id=%s Value=%s' . "\n", $name, (join '.', unpack( 'C*', $FlowRef->{$Id} ));
	                  } elsif (index($name, 'Port') ne -1) {
                                printf '  Id=%s Value=%s' . "\n", $name, unpack( 'S*', $FlowRef->{$Id} );
                          } else {
